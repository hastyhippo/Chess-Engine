...
void init_pesto_tables() {
    for (int t = 0; t < 6; t++) {
        for (int sq = 0; sq < 64; sq++) {
            int flipped = sq ^ 56;
            mg_table[WHITE][t][sq] = mg_piece_value[t] + mg_pesto[t][flipped];
            eg_table[WHITE][t][sq] = eg_piece_value[t] + eg_pesto[t][flipped];
            mg_table[BLACK][t][sq] = mg_piece_value[t] + mg_pesto[t][sq];
            eg_table[BLACK][t][sq] = eg_piece_value[t] + eg_pesto[t][sq];
        }
    }
}

static int evaluate_pesto(Board &b) {
    int mg[2] = {0, 0};
    int eg[2] = {0, 0};
    int gamePhase = 0;

    for (int sq = 0; sq < 64; sq++) {
        uint8_t pc = b.pieceOn(sq);
        if (pc == EMPTY_SQ) continue;
        Colour c = colour_of(pc);
        PieceType t = type_of(pc);
        mg[c] += mg_table[c][t][sq];
        eg[c] += eg_table[c][t][sq];
        gamePhase += gamephaseInc[t];
    }

    int side = b.getWhiteTurn() ? WHITE : BLACK;
...
static int evaluate_material(Board &b) {
    int score = 0;
...
int evaluate(Board &b) {
    return use_pesto ? evaluate_pesto(b) : evaluate_material(b);
}
