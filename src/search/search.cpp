...
int negamax(Board &b, int depth, int alpha, int beta, TimePoint time_limit, PVLine &pv) {
    ...
    for (Move m : possible_moves) {
        PVLine child_pv;
        b.make_move(m);
        int raw = negamax(b, depth - 1, -beta, -alpha, time_limit, child_pv);
        b.unmake_move(m);

        if (raw == OUTOFTIME) return OUTOFTIME;
        int score = -raw;
...
Move get_best_move(Board &b, int depth, int time_limit_ms) {
    ...
    for (int d = 1; d <= depth; d++) {
        ...
        for (Move m : moves) {
            PVLine child_pv;
            b.make_move(m);
            int raw = negamax(b, d - 1, -INF, INF, time_limit, child_pv);
            b.unmake_move(m);

            if (raw == OUTOFTIME) { out_of_time = true; break; }
            int value = -raw;
